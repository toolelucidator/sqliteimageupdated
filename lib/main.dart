...
class _HomePageState extends State<HomePage> {
  ...
  validate() {
    if (formKey.currentState!.validate()) {
      ...
      if (isUpdating) {
        Student stu =
            Student(currentUserId, name, apepa, apema, tel, email, photo_name);
        dbHelper.update(stu);
        setState(() {
          isUpdating = false;
        });
      } else {
        Student stu = Student(null, name, apepa, apema, tel, email, photo_name);
        dbHelper.save(stu);
      }
      clearData();
      refreshList();
  ...
  pickImageFromGallery() {
    ...
    imagePicker.pickImage(source: ImageSource.gallery, maxHeight: 480, maxWidth: 640).then((imgFile) async {
